const LOGOS = logos();

new Vue({
  el: "#app",
  data() {
    return { volumes: [], volumeIds: [], volumeIdx: 0 };
  },
  computed: {
    breadcrumbs() {
      let idx = this.volumeIdx;
      let last = this.volumes.length - 1;
      let next = idx === 0 ? null : this.volumeIds[idx - 1];
      let prev = idx === last ? null : this.volumeIds[idx + 1];
      let curr = this.volumeIds[idx];
      return { curr, prev, next };
    },
    currentVolume() {
      return this.volumes[this.volumeIdx];
    }
  },
  methods: {
    backgroundImage(image) {
      if (image.match(/^https?:\/\//)) return `url('${image}')`;
      return `url('assets/${image}')`;
    },
    cleanOrphan(desc) {
      return desc.replace(/ ([^ ]+)$/g, "&nbsp;$1");
    },
    linkHTML(url, key) {
      let logo = LOGOS[key];
      let html = `<a href="${url}" target="_blank" class="${
        logo ? "logo" : "text"
      }" title="${key}">`;
      html += logo ? logo : key;
      html += "</a>";
      return html;
    },
    onHashChange() {
      // Handling a hash in the
      let loc = window.location.hash;
      loc = loc && loc.match(/^#(\d+)$/) ? loc.replace(/^#/, "") : null;
      if (!loc) return;
      let idx = this.volumeIds.indexOf(loc);
      if (idx !== -1) this.volumeIdx = idx;
    },
    sortedLinks(links) {
      let keys = ["spotify", "apple", "discogs", "youtube", "wikipedia"];
      return keys.map(k => (links[k] ? [k, links[k]] : null)).filter(v => !!v);
    },
    uriToEmbedUrl(uri) {
      let [origin, type, id] = uri.split(":");
      if (origin === "spotify")
        return `https://open.spotify.com/embed/${type}/${id}`;
      if (origin === "youtube") return `https://www.youtube.com/embed/${type}`;
    }
  },
  mounted() {
    axios.get("data.json").then(({ data }) => {
      data.forEach(d => {
        this.volumes.push(d);
        this.volumeIds.push(d.volume);
      });
      window.onhashchange = this.onHashChange.bind(this);
      this.onHashChange();
    });
  },
  template: appTemplate()
});

function appTemplate() {
  return `
  <main>
    <nav>
      <ul>
        <li><a :href="'#' + breadcrumbs.prev" v-if="breadcrumbs.prev">{{ breadcrumbs.prev }}</a></li>
        <li><strong>{{ breadcrumbs.curr }}</strong></li>
        <li><a :href="'#' + breadcrumbs.next" v-if="breadcrumbs.next">{{ breadcrumbs.next }}</a></li>
      </ul>
    </nav>
    <section v-if="currentVolume">
      <article v-for="entry in currentVolume.entries">
        <div class="body">
          <iframe
            :src="uriToEmbedUrl(entry.embed_uri)"
            :key="'iframe-' + entry.embed_uri"
            width="300"
            height="300"
            frameborder="0"
            allowtransparency="true"
            allow="encrypted-media"
          ></iframe>
          <h3 class="track" v-html="cleanOrphan(entry.track)"></h3>
          <p class="album" v-html="cleanOrphan(entry.album)"></p>
          <p class="artist" v-html="cleanOrphan(entry.artist)"></p>
          <p class="description" v-html="cleanOrphan(entry.description)"></p>
          <div class="links">
            <span v-for="[key, url] in sortedLinks(entry.links)" v-html="linkHTML(url, key)"></span>
            <br v-if="entry.extra_links" />
            <span v-for="[title, url] in entry.extra_links" v-if="entry.extra_links">
              <a :href="url" target="_blank" class="text">{{ title }}</a>
            </span>
          </div>
        </div>
      </article>
    </section>
  </main>
  `;
}

function logos() {
  return {
    apple: `<svg xmlns="http://www.w3.org/2000/svg" width="28.188" height="28.195" viewBox="0 0 28.188 28.195" enable-background="new 0 0 28.188 28.195"><path fill="#FFF" d="M28.187 7.195c-.001-.867-.076-1.728-.282-2.574-.372-1.536-1.247-2.713-2.561-3.573A5.869 5.869 0 0 0 23.14.193a12.446 12.446 0 0 0-1.838-.177c-.048-.002-.098-.009-.146-.014H7.032l-.534.03c-.878.05-1.75.146-2.575.472C2.351 1.126 1.219 2.21.555 3.768.33 4.296.212 4.856.129 5.423c-.067.46-.105.922-.118 1.387-.001.036-.009.072-.012.108v14.359l.03.498c.059.957.181 1.908.584 2.787.764 1.668 2.041 2.764 3.799 3.291.495.149 1.005.22 1.519.268a20.62 20.62 0 0 0 1.957.072c4.319 0 8.638 0 12.957-.002.617 0 1.231-.039 1.844-.116.967-.124 1.876-.412 2.697-.952a5.919 5.919 0 0 0 2.208-2.594c.219-.495.344-1.021.434-1.555.134-.793.163-1.595.162-2.397-.002-4.461.001-8.921-.003-13.382m-7.545 4.687c0 2.237.001 4.474-.001 6.71 0 .49-.067.971-.285 1.417-.339.694-.894 1.13-1.63 1.339-.41.117-.829.184-1.256.203a2.193 2.193 0 0 1-2.284-1.804 2.208 2.208 0 0 1 1.221-2.374c.38-.188.786-.293 1.196-.381.444-.096.89-.18 1.332-.282.322-.074.537-.271.6-.606.015-.074.021-.151.021-.227.001-2.132.001-4.263-.001-6.394a.88.88 0 0 0-.03-.217c-.046-.177-.175-.285-.357-.275a4.05 4.05 0 0 0-.558.078c-.893.176-1.785.357-2.677.537l-2.734.552-1.614.326c-.019.004-.038.01-.056.015-.325.09-.443.239-.458.575-.002.051-.001.102-.001.154-.001 3.056 0 6.113-.002 9.168 0 .494-.055.982-.253 1.441-.327.754-.905 1.223-1.685 1.45a5.202 5.202 0 0 1-1.263.202c-1.126.042-2.061-.706-2.254-1.814-.167-.955.27-1.98 1.354-2.439.419-.176.859-.273 1.301-.365.337-.071.675-.135 1.011-.207.449-.098.684-.38.703-.839.002-.06.001-.117.001-.177 0-3.477 0-6.955.002-10.432 0-.145.015-.294.049-.435.082-.335.321-.526.641-.609.3-.078.605-.132.909-.194l2.583-.52 2.669-.539c.786-.159 1.572-.32 2.36-.475.258-.051.52-.103.78-.124.364-.029.614.2.651.566.009.087.014.175.014.263.002 2.245.002 4.489.001 6.733z"/></svg>`,
    discogs: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000"><path fill="#fff" d="M359.3 499.7c0-78.3 63.5-141.8 141.7-141.8 78.3 0 141.7 63.4 141.7 141.8 0 78.3-63.5 141.7-141.7 141.7-78.2 0-141.7-63.4-141.7-141.7zm6.2 0c.2 74.9 60.7 135.4 135.5 135.5 74.8 0 135.6-60.7 135.6-135.5-.1-74.9-60.7-135.5-135.6-135.6-74.8 0-135.5 60.7-135.5 135.6z"/><path fill="#fff" d="M523.5 499.7c0 12.3-10.1 22.3-22.4 22.3-12.3 0-22.4-10-22.4-22.3 0-12.4 10-22.4 22.4-22.4 12.3 0 22.4 10 22.4 22.4z"/><path fill="none" d="M501.1 314.8c-102.2 0-184.9 82.8-184.9 184.9 0 52.2 21.7 99.2 56.5 132.8l-.6-.6-48.2 52.1c16.8 16.2 35.8 30.1 56.6 41.3l34.8-62c25.7 13.5 54.9 21.2 85.9 21.2 102.1 0 184.9-82.8 184.9-184.9 0-49.4-19.5-94.2-51-127.3l48.5-52.2c-16.5-16.7-35.2-31.2-55.7-42.9l-34.2 62.4c-27.4-15.7-58.9-24.8-92.6-24.8zM262.4 750.5l-49.2 53.2c26.2 24.8 55.4 46.3 87.3 63.7l35.5-63.2c-26.8-14.6-51.5-32.8-73.6-53.7zM745.1 253.8l49.3-53.1c-26.3-25.8-56-48.2-88.3-66.4l-34.8 63.6c27 15.4 51.8 34.2 73.8 55.9zM689.7 313.4l49.1-52.8c-21.4-21.3-45.6-39.6-72-54.6l-34.6 63.2c21.1 12.1 40.5 27 57.5 44.2zM317.6 690.8l-48.9 52.9c21.5 20.5 45.6 38.2 71.9 52.5l35.3-62.8c-21.4-11.5-41-25.9-58.3-42.6z"/><path fill="#fff" d="M245.1 499.7c0 72.4 30.2 137.8 78.7 184.4L372 632l.6.6c-34.8-33.6-56.5-80.7-56.5-132.8 0-102.1 82.7-184.9 184.9-184.9 33.7 0 65.2 9.1 92.4 24.9l34.2-62.4C590.2 256 547 243.8 501 243.8c-141-.2-255.9 114.7-255.9 255.9zM686 499.7c0 102.1-82.8 184.9-184.9 184.9-31 0-60.2-7.7-85.9-21.2l-34.8 62c36 19.3 77.1 30.3 120.7 30.3 141.2 0 256-114.8 256-256 0-69.9-28.2-133.3-73.7-179.5L635 372.3c31.5 33.2 51 78 51 127.4z"/><path fill="#fff" d="M163.9 499.7c0 95.9 40.3 182.6 104.8 244.1l48.9-52.9c-50.3-48.3-81.7-116.1-81.7-191.2 0-146.3 119-265.2 265.2-265.2 47.6 0 92.3 12.7 131 34.8l34.6-63.2c-49-27.7-105.5-43.7-165.6-43.7-185.9 0-337.2 151.3-337.2 337.3zM766.4 499.7c0 146.3-119 265.2-265.2 265.2-45.3 0-87.9-11.4-125.3-31.5l-35.3 62.8c47.8 26 102.5 40.7 160.6 40.7 186 0 337.3-151.3 337.3-337.3 0-93.3-38.1-177.9-99.6-239.1l-49.1 52.8c47.3 48 76.6 113.8 76.6 186.4z"/><path fill="#fff" d="M847.6 499.7c0 191.1-155.4 346.5-346.5 346.5-59.7 0-116-15.2-165.1-41.9l-35.5 63.2c59.6 32.6 128 51.2 200.6 51.2 231 0 419-188 419-419 0-117-48.2-222.8-125.7-298.9l-49.3 53.1c63.3 62.8 102.5 149.7 102.5 245.8zM82.1 499.7c0 119.6 50.5 227.6 131.1 304l49.2-53.2c-66.3-63.2-107.8-152.2-107.8-250.8 0-191.1 155.4-346.5 346.5-346.5 61.8 0 119.8 16.3 170.1 44.8l34.8-63.6c-60.6-34.2-130.5-53.7-204.9-53.7-231-.1-419 187.9-419 419z"/><path fill="#fff" d="M848.8 142.3L800.7 194c79.3 77.8 128.7 186 128.7 305.7 0 236.1-192.1 428.2-428.2 428.2-74.3 0-144.2-19-205.1-52.4l-35.6 63.4c71.2 39.1 152.9 61.4 239.8 61.5 276.1 0 500.1-223.9 500.1-500-.2-140.5-58.3-267.3-151.6-358.1zM72.9 499.7C72.9 263.5 265 71.4 501.1 71.4c76 0 147.4 20 209.4 54.9l33.9-62C674.4 24.9 594 1.9 508.3.5h-8.1C225.7 1 3.3 221.7.3 495.7v4.8C.6 643.8 61 772.8 157.6 863.9l49.4-53.4C124.5 732.4 72.9 621.9 72.9 499.7z"/></svg>`,
    spotify: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><path d="M24.008.007c-13.254 0-24 10.745-24 24s10.746 24 24 24 24-10.747 24-24c0-13.255-10.745-24-24-24m-3.521 12.526c7.05 0 14.461 1.45 19.877 4.63.73.414 1.237 1.038 1.237 2.18 0 1.309-1.052 2.25-2.264 2.25-.489 0-.779-.12-1.237-.37-4.344-2.605-11.09-4.04-17.604-4.04-3.254 0-6.56.33-9.587 1.153-.35.088-.79.262-1.231.262-1.28 0-2.263-1.013-2.263-2.293 0-1.304.808-2.036 1.68-2.294 3.42-1.01 7.24-1.478 11.39-1.478m-.488 8.247c6.281 0 12.358 1.565 17.14 4.424.8.459 1.095 1.043 1.095 1.897a1.867 1.867 0 0 1-1.87 1.879c-.522 0-.849-.21-1.203-.416-3.904-2.317-9.315-3.856-15.24-3.856-3.038 0-5.661.426-7.832 1-.468.129-.73.267-1.168.267a1.88 1.88 0 0 1-1.875-1.882c0-1.021.495-1.723 1.493-2 2.698-.741 5.453-1.313 9.463-1.313m.329 7.837c5.251 0 9.931 1.205 13.96 3.619.598.35.95.708.95 1.595 0 .865-.704 1.5-1.493 1.5-.388 0-.653-.134-1.016-.355-3.478-2.104-7.815-3.211-12.417-3.211-2.568 0-5.153.329-7.57.833-.393.085-.888.237-1.183.237a1.498 1.498 0 0 1-1.52-1.508c0-1.01.583-1.511 1.307-1.65 2.964-.675 5.917-1.06 8.983-1.06" fill="#fff"/></svg>`,
    wikipedia: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="128" height="128" stroke-miterlimit="10" shape-rendering="geometricPrecision" fill-rule="evenodd"><path fill="#fff" d="M120.85 29.21c0 .41-.13.78-.38 1.12-.26.33-.53.5-.84.5-2.49.24-4.54 1.04-6.12 2.41-1.59 1.36-3.22 3.97-4.91 7.81L82.8 99.19c-.17.54-.64.81-1.42.81-.61 0-1.08-.27-1.42-.81L65.49 68.93 48.85 99.19c-.34.54-.81.81-1.42.81-.74 0-1.23-.27-1.47-.81L20.61 41.05c-1.58-3.61-3.25-6.13-5.01-7.56-1.75-1.43-4.2-2.32-7.33-2.66-.27 0-.53-.14-.76-.43-.24-.28-.36-.61-.36-.98 0-.95.27-1.42.81-1.42 2.26 0 4.62.1 7.09.3 2.29.21 4.45.31 6.47.31 2.06 0 4.49-.1 7.29-.31 2.93-.2 5.53-.3 7.79-.3.54 0 .81.47.81 1.42 0 .94-.17 1.41-.5 1.41-2.26.17-4.04.75-5.34 1.72-1.3.98-1.95 2.26-1.95 3.85 0 .81.27 1.82.81 3.03l20.95 47.31 11.89-22.46-11.08-23.23c-1.99-4.14-3.63-6.82-4.91-8.02-1.28-1.19-3.22-1.93-5.82-2.2-.24 0-.46-.14-.68-.43-.22-.28-.33-.61-.33-.98 0-.95.23-1.42.71-1.42 2.26 0 4.33.1 6.22.3 1.82.21 3.76.31 5.82.31 2.02 0 4.16-.1 6.42-.31 2.33-.2 4.62-.3 6.88-.3.54 0 .81.47.81 1.42 0 .94-.16 1.41-.5 1.41-4.52.31-6.78 1.59-6.78 3.85 0 1.01.52 2.58 1.57 4.7l7.33 14.88 7.29-13.61c1.01-1.92 1.52-3.54 1.52-4.86 0-3.1-2.26-4.75-6.78-4.96-.41 0-.61-.47-.61-1.41 0-.34.1-.66.3-.96.21-.31.41-.46.61-.46 1.62 0 3.61.1 5.97.3 2.26.21 4.12.31 5.57.31 1.04 0 2.58-.09 4.6-.26 2.56-.23 4.71-.35 6.43-.35.4 0 .6.4.6 1.21 0 1.08-.37 1.62-1.11 1.62-2.63.27-4.75 1-6.35 2.18-1.6 1.18-3.6 3.86-5.99 8.04l-9.72 17.97 13.16 26.81 19.43-45.18c.67-1.65 1.01-3.17 1.01-4.55 0-3.31-2.26-5.06-6.78-5.27-.41 0-.61-.47-.61-1.41 0-.95.3-1.42.91-1.42 1.65 0 3.61.1 5.87.3 2.09.21 3.85.31 5.26.31 1.49 0 3.21-.1 5.16-.31 2.03-.2 3.85-.3 5.47-.3.47 0 .71.4.71 1.21z"/></svg>`,
    youtube: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 215.094 215.094" enable-background="new 0 0 215.094 215.094"><path d="M28.302 32.547C12.673 32.547 0 45.22 0 60.849v93.396c0 15.629 12.673 28.302 28.302 28.302h158.491c15.629 0 28.301-12.673 28.301-28.302V60.849c0-15.629-12.672-28.302-28.301-28.302H28.302zm62.245 113.036V69.511l50 38.036-50 38.036z" fill="#fff"/></svg>`
  };
}
